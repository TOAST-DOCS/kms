...
```mermaid
sequenceDiagram
    participant App as ユーザーアプリケーション
    participant DB as データストレージ
    participant SKM as Secure Key Manager

    rect rgb(200, 230, 255)
    note right of App: 暗号化キー生成プロセス

    App->>SKM: 1. KEK生成 (対称鍵生成)
    SKM->>SKM: KEK生成
    App->>SKM: 2. DEK生成リクエスト (create-local-key)
    SKM->>SKM: DEK生成
    SKM-->>App: 平文DEKとDEK暗号文を返却
    App->>DB: 3. DEK暗号文を保存<br/>(SKMまたはユーザーDBに保存可能)
    end

    rect rgb(248, 244, 211)
    note left of SKM: データ暗号化/復号プロセス

    App->>DB: 1. 保存されたDEK暗号文を照会
    DB-->>App: DEK暗号文を返却
    App->>SKM: 2. SKM復号APIでDEK暗号文を復号
    SKM-->>App: 復号された平文DEKを返却
    App->>App: 3. 平文DEKで<br/>ユーザーデータを暗号化/復号
    end

    rect rgb(230, 255, 230)
    note left of SKM: キー自動ローテーション
    SKM->>SKM: KEK v1 → KEK v2 ローテーション<br/>(自動ローテーション)

    note right of App: キー手動ローテーション
    App->>SKM: キーローテーションリクエスト (KEK v1 → KEK v2)<br/>(手動ローテーション)

    note right of App: キーローテーション後のDEK暗号文変更の反映
    App->>DB: 1. 既存のDEK暗号文を照会
    DB-->>App: DEK暗号文を返却
    App->>SKM: 2. SKM復号APIでDEK暗号文を復号
    SKM-->>App: 平文DEKを返却
    App->>SKM: 3. SKM暗号化APIで<br/>平文DEKをローテーションされた新規KEKで暗号化
    SKM-->>App: 新規DEK暗号文を返却
    App->>DB: 4. 保存していたDEK暗号文を新規DEK暗号文に修正

    note over App, SKM: ✅ 旧バージョン(KEK v1)で暗号化されたDEK暗号文: KEK v1が維持されれば復号可能<br/>✅ ローテーション後の新規DEK暗号文: KEK v2で暗号化/復号<br/>✅ アプリケーションコード: 変更不要<br/>✅ 実際のユーザーデータ: 再暗号化不要
