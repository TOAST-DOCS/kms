...
!!! tip "알아두기"
    이 가이드는 이미 SKM을 사용 중인 분들을 위한 가이드입니다. 처음 사용하는 경우 [SKM 가이드](./overview)를 먼저 읽어주세요.

## 주요 용어

이 가이드에서 사용하는 주요 용어를 먼저 이해하면 내용을 더 쉽게 파악할 수 있습니다.

* **키 회전(key rotation)**: 보안을 강화하기 위해 암호화 키를 주기적으로 새로운 키로 교체하는 작업입니다. 마치 비밀번호를 정기적으로 변경하는 것과 같은 개념입니다.
* **봉투 암호화(envelope encryption)**: 데이터를 두 단계로 암호화하는 방법입니다. 중요한 문서를 봉투에 넣고, 그 봉투를 다시 금고에 보관하는 것처럼 데이터를 이중으로 보호합니다.
* **DEK(data encryption key, 데이터 암호화 키)**: 실제 데이터를 암호화하는 데 사용하는 키입니다. 봉투 암호화에서 '봉투' 역할을 합니다.
* **KEK(key encryption key, 키 암호화 키)**: DEK를 암호화하는 데 사용하는 키입니다. SKM에서 관리하는 대칭 키가 이 역할을 합니다. 봉투 암호화에서 '금고' 역할을 합니다.
...
SKM의 키 회전 기능을 사용하면 키 ID를 변경하지 않고 키값만 갱신할 수 있어, 애플리케이션 코드 수정 없이 보안을 강화할 수 있습니다.
...
!!! tip "알아두기"
    SKM은 최소 30일부터 자동 회전 주기를 설정할 수 있습니다.
...
봉투 암호화는 데이터를 두 단계로 암호화하는 방법입니다. 일반적인 암호화 방식과 어떻게 다른지 비교해보겠습니다.
...
[DEK] → [KEK로 암호화] → [암호화된 DEK]           (KEK는 SKM 저장, 암호화된 DEK는 DB 또는 SKM 저장)
```

봉투 암호화는 일반 암호화 방식에 비해 다음과 같은 보안상 이점이 있습니다.

* **키 노출 범위 최소화**: KEK는 SKM에서 안전하게 보관되며 평문으로 노출되지 않습니다. 반면 일반 암호화 방식에서는 암호화 키가 애플리케이션 메모리에 평문으로 존재해야 하므로 노출 위험이 더 큽니다.
* **키 유출 시 대응 용이**: DEK가 암호화된 상태로 저장되므로, KEK만 즉시 회전하면 유출된 암호화된 DEK는 더 이상 복호화할 수 없습니다. 일반 암호화 방식에서는 키 유출 시 모든 데이터를 즉시 재암호화해야 합니다.
...
아파트 현관 비밀번호를 정기적으로 변경해야 하는 상황을 생각해봅시다.

* **일반 방식**: 현관문(데이터) 자체를 뜯어내고 새 문으로 교체 → 100세대라면 100개 문을 모두 교체해야 함(공사가 너무 큼)
* **봉투 방식**: 현관문 열쇠(DEK)를 보관해 둔 작은 보관함의 비밀번호(KEK)만 변경 → 문은 그대로 두고, 열쇠를 꺼내는 방법만 바꾸면 되므로 매우 간편함
...
* **KEK 회전의 장점**: KEK를 회전해도 실제 데이터는 그대로 두고, 암호화된 DEK만 새 KEK 버전으로 재암호화하면 됩니다.
* **하위 호환성**: SKM은 여러 버전의 KEK를 동시에 관리하므로, 이전 버전 KEK로 암호화된 DEK도 자동으로 복호화할 수 있습니다.
...
봉투 암호화 방식
키 회전 시 → 실제 데이터는 그대로, DEK는 이미 암호화되어 있음
...
봉투 암호화 형태로 SKM을 이용할 경우 다음과 같은 흐름을 갖는 것을 추천합니다.
...
sequenceDiagram
    ...
    note right of App: 암호화 키 생성 과정

    App->>SKM: 1. KEK 생성 (대칭 키 생성)
    SKM->>SKM: KEK 생성
    App->>SKM: 2. DEK 생성 요청 (create-local-key)
    SKM->>SKM: DEK 생성
    SKM-->>App: 평문 DEK와 DEK 암호문 반환
    App->>DB: 3. DEK 암호문을 저장<br/>(SKM 또는 사용자 DB에 저장 가능)
    end

    rect rgb(248, 244, 211)
    note left of SKM: 데이터 암/복호화 과정
    ...
    note left of SKM: 키 자동 회전
    SKM->>SKM: KEK v1 → KEK v2 회전<br/>(자동 회전)

    note right of App: 키 수동 회전
    App->>SKM: 키 회전 요청 (KEK v1 → KEK v2)<br/>(수동 회전)
    ...
    note over App, SKM: ✅ 암호화된 DEK: KEK v1이 삭제되지 않으면 복호화 가능<br/>✅ 회전 후의 신규 DEK 암호문: KEK v2로 암/복호화<br/>✅ 애플리케이션 코드: 변경 불필요<br/>✅ 실제 사용자 데이터: 재암호화 불필요
...
!!! danger "주의"
    키가 필요한 모든 요청에 대하여 캐싱하지 않고 SKM API를 호출하면 SKM 서비스 장애가 발생할 수 있습니다. 적절히 캐싱하여 사용해야 합니다.
...
1. 데이터 생성 시점(년월)별로 SKM에서 대칭 키를 생성하고 자동 회전을 설정합니다.
...
4. 암호화된 데이터와 함께 사용된 키 ID와 키 버전 정보를 함께 저장합니다(예: `keyId: abc123..., version: 0`).
5. 복호화 시에는 저장된 키 ID와 버전 정보를 사용하여 데이터를 복호화합니다.

!!! tip "알아두기"
    SKM의 키는 UUID 형식의 키 ID와 0부터 시작하는 버전으로 관리됩니다. 키 회전 시 버전이 0 → 1 → 2로 증가하므로, 어느 버전으로 암호화했는지 애플리케이션에서 반드시 기록해야 합니다.

**장점**

* 키 유출 시 해당 기간 데이터만 영향
* 오래된 데이터는 키 삭제로 완전한 데이터 파기 가능
* 규제 준수: 데이터 보관 기간 만료 시 키 삭제로 암호학적 파기
...
1. SKM에서 여러 개의 대칭 키를 생성합니다(예: 10개).
...
4. 암호화된 데이터와 함께 사용된 키 ID와 키 버전 정보를 함께 저장합니다.
5. 복호화 시에는 저장된 키 ID와 버전 정보를 사용하여 데이터를 복호화합니다.

**방법 2: 고객 등급별 분할**

1. 고객 등급별로 SKM에서 대칭 키를 생성합니다(예: VIP용, PREMIUM용, STANDARD용).
2. 애플리케이션에서 고객 등급별 키 ID 매핑 정보를 관리합니다(예: `VIP → 키 ID: abc123...`, `PREMIUM → 키 ID: def456...`).
3. 암호화된 데이터와 함께 사용된 키 ID와 키 버전 정보를 함께 저장합니다.
...
1. SKM API를 사용하여 현재 활성화된 모든 키 목록을 조회합니다.
...
1. SKM 콘솔에서 키 저장소 선택
2. **키 관리** 메뉴 클릭
3. 회전할 대칭 키 또는 비대칭 키 선택
4. 상세 정보 창에서 **자동 회전 주기** 설정
...
현재 SKM의 대칭 키를 **데이터를 직접 암호화하는 용도**로 사용 중이라면, **키 회전을 적용하기 전에 반드시 봉투 암호화로 전환**해야 합니다.
...
```
사용자 데이터 → SKM의 대칭 키로 직접 암호화 → 암호화된 데이터 저장
```

이 상태에서 키 회전을 적용하면 다음과 같은 문제가 발생합니다.

1. **키 버전이 0 → 1로 변경됨**
2. **버전 0으로 암호화된 기존 데이터는 버전 1로 복호화 시도**
...
1. 봉투 암호화 구조로 시스템 재설계
2. 기존 데이터를 새로운 구조로 마이그레이션
3. 마이그레이션 완료 후 키 회전 적용
...
높아지는 보안 요구사항에 대응하기 위해, 이 가이드에서는 SKM의 키 회전 기능을 활용한 두 가지 보안 강화 전략을 소개했습니다. 서비스 환경과 데이터 특성에 따라 다음과 같이 필수 또는 선택적으로 적용할 수 있습니다.
...
SKM의 키 회전 기능을 적절히 활용하면 다음과 같은 효과를 얻을 수 있습니다.

1. **애플리케이션 코드 변경 없이** 보안 수준 향상
2. **자동화된 키 관리**로 운영 부담 감소
3. **보안 컴플라이언스** 요구사항 충족
4. **보안 사고 발생 시 피해 범위** 최소화

키 회전은 일회성 작업이 아닌 **지속적인 보안 프로세스**입니다. 서비스 규모와 보안 요구사항에 맞게 전략을 선택하고, 정기적인 검토와 개선을 통해 보안 수준을 꾸준히 향상시킬 수 있습니다.
