...
[DEK] → [KEK로 암호화] → [DEK 암호문]            (KEK는 SKM 저장, DEK 암호문은 DB 또는 SKM 저장)
```
...
* **키 유출 시 대응 용이**: DEK가 암호화된 상태로 저장되므로, KEK만 즉시 회전하면 유출된 DEK 암호문은 더 이상 복호화할 수 없습니다. 일반 암호화 방식에서는 키 유출 시 모든 데이터를 즉시 재암호화해야 합니다.
...
* **KEK 회전의 장점**: KEK를 회전해도 실제 데이터는 그대로 두고, DEK 암호문만 새 KEK 버전으로 재암호화하면 됩니다.
* **하위 호환성**: SKM은 여러 버전의 KEK를 동시에 관리하므로, 이전 버전 KEK로 DEK 암호문도 자동으로 복호화할 수 있습니다.
...
sequenceDiagram
    ...
    note over App, SKM: ✅ DEK 암호문: KEK v1이 삭제되지 않으면 복호화 가능<br/>✅ 회전 후의 신규 DEK 암호문: KEK v2로 암/복호화<br/>✅ 애플리케이션 코드: 변경 불필요<br/>✅ 실제 사용자 데이터: 재암호화 불필요
