...
* **KEK(key encryption key, 키 암호화 키)**: DEK를 암호화하는 데 사용하는 키입니다. Secure Key Manager 서비스에서 관리하는 대칭 키가 이 역할을 합니다. 봉투 암호화에서 '금고' 역할을 합니다.
...
Secure Key Manager 서비스의 키 회전 기능을 사용하면 키 ID를 변경하지 않고 키값만 갱신할 수 있어, 애플리케이션 코드 수정 없이 보안을 강화할 수 있습니다.
...
[DEK] → [KEK로 암호화] → [DEK 암호문]            (KEK는 Secure Key Manager 서비스에 저장, DEK 암호문은 DB 또는 Secure Key Manager 서비스에 저장)
```

봉투 암호화는 일반 암호화 방식에 비해 다음과 같은 보안상 이점이 있습니다.

* **키 노출 범위 최소화**: KEK는 Secure Key Manager 서비스에서 안전하게 보관되며 사용자의 코드로 노출되지 않습니다.
* **키 관리 집중화**: 중요한 KEK는 Secure Key Manager 서비스에서 중앙 집중 관리되고, DEK는 각 데이터마다 다르게 생성할 수 있어 피해 범위를 최소화할 수 있습니다.
* **감사 추적 강화**: Secure Key Manager 서비스가 모든 KEK 사용 내역을 기록하므로, 키 사용에 대한 감사 추적이 용이합니다.
...
* **이전 버전 호환**: Secure Key Manager 서비스는 여러 버전의 KEK를 동시에 관리하므로, 이전 버전 KEK로 암호화된 DEK 암호문도 자동으로 복호화할 수 있습니다.
...
봉투 암호화 형태로 Secure Key Manager 서비스를 이용할 경우 다음과 같은 흐름을 갖는 것을 추천합니다.
...
1. 데이터 생성 시점(년월)별로 Secure Key Manager 서비스에서 대칭 키를 생성하고 자동 회전을 설정합니다.
...
!!! tip "알아두기"
    Secure Key Manager 서비스의 키는 UUID 형식의 키 ID와 0부터 시작하는 버전으로 관리됩니다. 키 회전 시 버전이 0 → 1 → 2로 증가하므로, 어느 버전으로 암호화했는지 애플리케이션에서 반드시 기록해야 합니다.
...
1. Secure Key Manager 서비스에서 여러 개의 대칭 키를 생성합니다(예: 10개).
...
1. 고객 등급별로 Secure Key Manager 서비스에서 대칭 키를 생성합니다(예: VIP용, PREMIUM용, STANDARD용).
...
1. Secure Key Manager 콘솔에서 키 저장소 선택
2. **키 관리** 메뉴 클릭
...
현재 Secure Key Manager 서비스의 대칭 키를 **데이터를 직접 암호화하는 용도**로 사용 중이라면, **키 회전을 적용하기 전에 반드시 봉투 암호화로 전환**해야 합니다.
...
```
사용자 데이터 → Secure Key Manager 서비스의 대칭 키로 직접 암호화 → 암호화된 데이터 저장
```
...
높아지는 보안 요구사항에 대응하기 위해, 이 가이드에서는 Secure Key Manager 서비스의 키 회전 기능을 활용한 두 가지 보안 강화 전략을 소개했습니다. 서비스 환경과 데이터 특성에 따라 다음과 같이 필수 또는 선택적으로 적용할 수 있습니다.
...
Secure Key Manager 서비스의 키 회전 기능을 적절히 활용하면 다음과 같은 효과를 얻을 수 있습니다.
