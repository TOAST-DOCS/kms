...
* **하위 호환성**: SKM은 여러 버전의 KEK를 동시에 관리하므로, 이전 버전 KEK로 암호화된 DEK 암호문도 자동으로 복호화할 수 있습니다.
...
sequenceDiagram
    ...
    note over App, SKM: ✅ 이전 버전(KEK v1)으로 암호화된 DEK 암호문: KEK v1이 유지되면 복호화 가능<br/>✅ 회전 후의 신규 DEK 암호문: KEK v2로 암/복호화<br/>✅ 애플리케이션 코드: 변경 불필요<br/>✅ 실제 사용자 데이터: 재암호화 불필요
